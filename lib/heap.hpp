...
class Heap {
 ...
 public:
  Heap(int size) { construct_heap(size); }

  int *get_heap() { return internal_arr; }
  int get_size() { return heap_size; }
  void set_size(int size) {
    if (size > -1) heap_size = size;
  }
...
class MaxHeap : public Heap {
 private:
  void construct_from_array(int *arr, int size) {
    heap_size = size;
    for (int i = 0; i < size; ++i) internal_arr[i] = arr[i];
...
void MaxHeap::increase_key(int index, int key) {
  if (internal_arr[index] > key) return;
...
int MaxHeap::remove(int index) {
  if (heap_size < 1 || index > heap_size) return -1;
